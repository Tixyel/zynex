...
type CommandEvent =
  | { provider: 'twitch'; data: Twitch$Message }
  | { provider: 'youtube'; data: Youtube$Message }
...
export class Command {
  ...
  verify(nickname: string, roles: string[], args: string[]): boolean {
    ...
    if (
      ...
      this.permissions.some((p) => roles.map((r) => r.toLowerCase()).includes(p.toLowerCase()))
    ) {
      return true
    }
  ...
  static execute(received: CommandEvent): boolean {
    ...
    try {
      if (
        ...
        const command = window.client.actions.commands.find(
          (c) => data.event.data.text.replace(c.prefix, '').split(' ')[0] === c.name,
        )
